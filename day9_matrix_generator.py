...
def gen_matrix(width, height, num_red_tiles):
    ...
    while True:
        ...
        if prev_tile:
            pick_w_h = random.randint(0, 1)
            if pick_w_h == 0: # Use previous node's w
                w = prev_tile[0]
            else: # Use prev node's h
                h = prev_tile[1] 
        red_tiles.append((w, h))

        if len(red_tiles) == num_red_tiles:
            break
